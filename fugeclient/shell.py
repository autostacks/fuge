...
"""
Command-line interface to the OpenStack Fuge API.
"""
...
from fugeclient.common.apiclient import auth
from fugeclient.common import cliutils
from fugeclient import exceptions as exc
from fugeclient.v1 import client as client_v1
from fugeclient.v1 import shell as shell_v1
# from fugeclient import version
...
class SecretsHelper(object):
    ...
    def save(self, auth_token, management_url, tenant_id):
        ...
        value = "|".join([str(auth_token),
                          str(management_url),
                          str(tenant_id)])
        keyring.set_password("fugeclient_auth", self._make_key(), value)
    ...
    def management_url(self):
        ...
        management_url = None
        try:
            block = keyring.get_password('fugeclient_auth',
                                         self._make_key())
            if block:
...
class FugeClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(FugeClientArgumentParser, self).__init__(*args, **kwargs)
...
class OpenStackFugeShell(object):

    def get_base_parser(self):
        parser = FugeClientArgumentParser(
            prog='fuge',
            description=__doc__.strip(),
            epilog='See "fuge help COMMAND" '
...
#            help="Set HTTP call timeout (in seconds)")
        ...
        parser.add_argument('--fuge-api-version',
                            metavar='<fuge-api-ver>',
                            default=cliutils.env(
                                'FUGE_API_VERSION',
                                default=DEFAULT_API_VERSION),
                            help='Accepts "api", '
                                 'defaults to env[FUGE_API_VERSION].')
        parser.add_argument('--fuge_api_version',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--insecure',
                            default=cliutils.env('FUGECLIENT_INSECURE',
                                                 default=False),
                            action='store_true',
    ...
    def main(self, argv):
        ...
        subcommand_parser = (
            self.get_subcommand_parser(options.fuge_api_version)
        )
...
# NOTE: The Fuge client authenticates when you create it. So instead of
...
#       does, we just create the client later.
    ...
    def do_bash_completion(self, _args):
        ...
        fuge.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        OpenStackFugeShell().main(map(encodeutils.safe_decode, sys.argv[1:]))
