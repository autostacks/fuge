...
from fuge.api.controllers.v1 import services as fuge_service
from fuge.tests import base
from fuge.tests.unit.api import utils as apiutils


class TestServiceObject(base.BaseTestCase):

    def setUp(self):
        super(TestServiceObject, self).setUp()
        self.rpc_dict = apiutils.fuge_service_get_data()

    def test_msvc_obj_fields_filtering(self):
        """Test that it does filtering fields """
        self.rpc_dict['fake-key'] = 'fake-value'
        msvco = fuge_service.Service("up", **self.rpc_dict)
...
# class TestServiceController(api_base.FunctionalTest):
...
#            elem = apiutils.fuge_service_get_data()
#            elem['id'] = i + 1
#            rec = db_rec(elem)
#            reclist.append(rec)
#        return reclist
